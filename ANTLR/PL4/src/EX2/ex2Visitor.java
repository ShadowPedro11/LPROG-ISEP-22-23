// Generated from C:/Users/0941p/OneDrive/Ambiente de Trabalho/ISEP/ISEP_2�ano/2�Semestre/LPROG/ANTLR/PL4/src/EX2\ex2.g4 by ANTLR 4.12.0
package EX2;
import org.antlr.v4.runtime.tree.ParseTreeVisitor;

/**
 * This interface defines a complete generic visitor for a parse tree produced
 * by {@link ex2Parser}.
 *
 * @param <T> The return type of the visit operation. Use {@link Void} for
 * operations with no return type.
 */
public interface ex2Visitor<T> extends ParseTreeVisitor<T> {
	/**
	 * Visit a parse tree produced by {@link ex2Parser#ex}.
	 * @param ctx the parse tree
	 * @return the visitor result
	 */
	T visitEx(ex2Parser.ExContext ctx);
	/**
	 * Visit a parse tree produced by {@link ex2Parser#coisas}.
	 * @param ctx the parse tree
	 * @return the visitor result
	 */
	T visitCoisas(ex2Parser.CoisasContext ctx);
}